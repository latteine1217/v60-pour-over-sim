...
NOTE: P0/P1 重構後，`scan_wetbed_structure` 已移除——χ 結構態被併入 `f_post`，
      不再以獨立 ODE 形式存在；對應的 identifiability 分析改由
      ``pour_over.identifiability.analyze_fit_identifiability`` 在
      ``wetbed_irr_gain`` / ``wetbed_rev_gain`` 軸上進行。
"""
